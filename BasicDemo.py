...
# Obtain the index of the selected device information, and parse it through the characters between []
...
if __name__ == "__main__":
    ...
    global isGrabbing
    isGrabbing = False
    global isCalibMode  # 是否是标定模式（获取原始图像）Whether it is a calibration mode (get the original image)
    isCalibMode = True
    ...
    def set_software_trigger_mode():
        ...
        if ret != 0:
            ...
            # Trigger một lần để đọc mã vạch từ frame và hiển thị nội dung
            if isGrabbing:
               print('grabbing')
               ret = obj_cam_operation.Trigger_once()
               if ret == 0:
                  ret, frame = obj_cam_operation.Read_frame()
                  if ret == 0:
                     read_qr_code_and_display(frame)
                  else:
                     strError = "Read frame failed ret:" + ToHexStr(ret)
                     QMessageBox.warning(mainWindow, "Error", strError, QMessageBox.Ok)
               else:
    ...
    def set_param():
        ...
        ret = obj_cam_operation.Set_parameter(frame_rate, exposure, gain)# set thong so 
        if ret != MV_OK:
