    ...
    def Work_thread(self, winHandle):
        ...
        while True:
            ...
            if 0 == ret:
                ...
                #print(type(self.buf_grab_image))
                #cái này là lấy biến hình từ camera chuyển qua định dạng openCV để đọc code 
                frame_image = cv2.cvtColor(
                        np.array(self.buf_save_image).reshape(self.st_frame_info.nHeight, self.st_frame_info.nWidth), 
                        cv2.COLOR_BAYER_RG2RGB
                )

                
                # Sử dụng thư viện ZBar để đọc mã QR từ hình ảnh
                decoded_objects = pyzbar.decode(frame_image)
                result =str(decoded_objects)
                print("QR code content:", result)
                if decoded_objects:
                   for obj in decoded_objects:
                      x, y, w, h = obj.rect.left, obj.rect.top, obj.rect.width, obj.rect.height
                      cv2.rectangle(frame_image, (x, y), (x + w, y + h), (0, 255, 0), 30)
                # file_name = "frame.jpg"
                # # # Lưu ảnh vào thư mục hiện tại (cùng thư mục với file code)
                # cv2.imwrite(file_name, frame_image)
                print("lay mot frame: Width[%d], Height[%d], nFrameNum[%d]"
            ...
            #đổi dòng này đê chuyển từ open cv qua định dạng MVS nhé hehe
            ...
            stDisplayParam.enPixelType = PixelType_Gvsp_BGR8_Packed  # Đảm bảo định dạng là BGR
            #stDisplayParam.enPixelType = PixelType_Gvsp_RGB8_Packed  # Chuyển đổi thành dạng RGB
            stDisplayParam.pData = frame_image.ctypes.data_as(POINTER(c_ubyte))  # Sử dụng địa chỉ dữ liệu của frame_image
            stDisplayParam.nDataLen = self.st_frame_info.nFrameLen
            self.obj_cam.MV_CC_DisplayOneFrame(stDisplayParam)#này là đối tượng hiển thị lên màn hìnhh 
