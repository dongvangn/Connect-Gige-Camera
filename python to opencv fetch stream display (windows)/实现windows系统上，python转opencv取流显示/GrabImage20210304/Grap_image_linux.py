...
def image_show(image):
    # hàm này nhận vào một image từ camera
    print("okokoo")
    image = cv2.resize(image, (600, 400), interpolation=cv2.INTER_AREA)
    cv2.imshow('test', image)
    k = cv2.waitKey(1) & 0xff
    # Kiểm tra phím 'q' (mã ASCII 113)
    if k == ord('q'):
        exit_flag = True
...
def work_thread_1(cam=0, pData=0, nDataSize=0):
    ...
    while not exit_flag:
        stFloatParam_exposureTime =12
        #cam.MV_CC_SetFloatValue("ExposureTime", float(22))
        ...
        if ret == 0:
            ...
            # exposure_time_us = 1000  # Đặt giá trị thời gian mở khẩu theo đơn vị micro giây (us)
            # ret = cam.MV_CC_SetFloatValue("ExposureTime", exposure_time_us)
            ...
            if ret != 0:
                ...
                #print("chuyển đổi qua định dạng opemcv")
                if IsImageColor(stFrameInfo.enPixelType) == 'mono':#chuyển qua định dạng opecv 
                    img_buff = np.zeros((stFrameInfo.nHeight, stFrameInfo.nWidth), dtype=np.uint8)
                    ctypes.memmove(img_buff.ctypes.data, stConvertParam.pDstBuffer, stConvertParam.nDstLen)
                    img_buff = img_buff.reshape((stFrameInfo.nHeight, stFrameInfo.nWidth))
                    image = cv2.resize(img_buff, (600, 400), interpolation=cv2.INTER_AREA)
                    cv2.imshow('test', image)
                    #print(a)
                    k = cv2.waitKey(1) & 0xff
                    if k == ord('q'):#nhấn q để thoát hỏi vòng lặp
                        exit_flag = True
