# ！/liubaoliang/bin/python3
#Author:Mat
# -*- coding: utf-8 -*-
# @Time      :2021/2/23 14:20
# @Author    :Mat
# @Site      :
# @File      :Mv_CameraControl_function.py
# @Seftware  :PyCharm
import os
import sys
import numpy as np
...
# 保存视频???????????????????????????????????????????????
def save_video( data , stFrameInfo):
    fps = 10
    size = (stFrameInfo.nWidth , stFrameInfo.nHeight)
    # 保存.flv视频文件
    # videoWriter = cv2.VideoWriter("text.flv", cv2.VideoWriter_fourcc("F", "L", "V", "1"), fps, size)  # fps表示帧率
    # 保存.avi视频文件
    videoWriter = cv2.VideoWriter("text.avi", cv2.VideoWriter_fourcc("I", "4", "2", "0"), fps, size)  # fps表示帧率
    data = data.reshape(stFrameInfo.nHeight, stFrameInfo.nWidth, -1)
    image = cv2.cvtColor(data, cv2.COLOR_BAYER_RG2RGB)
...
def access_get_image(cam , active_way = "getImagebuffer"):
    """
    :param cam:     相机实例
    :active_way:主动取流方式的不同方法 分别是（getImagebuffer）（getoneframetimeout）（getimageforRGB）（getimageforBGR）
    :return:
    """
    if active_way == "getImagebuffer":
        ...
        while True:
            ...
            if None != stOutFrame.pBufAddr and 0 == ret and stOutFrame.stFrameInfo.enPixelType == 17301505:       # Mono8 格式
                ...
                data = np.frombuffer(pData, count=int(stOutFrame.stFrameInfo.nWidth * stOutFrame.stFrameInfo.nHeight),dtype=np.uint8)
                image_control(data=data, stFrameInfo=stOutFrame.stFrameInfo)
            elif None != stOutFrame.pBufAddr and 0 == ret and stOutFrame.stFrameInfo.enPixelType == 35127316:     # RGB8_Packed 格式
                print("get one frame: Width[%d], Height[%d], nFrameNum[%d]" % (stOutFrame.stFrameInfo.nWidth, stOutFrame.stFrameInfo.nHeight, stOutFrame.stFrameInfo.nFrameNum))
                ...
                image_control(data=data, stFrameInfo=stOutFrame.stFrameInfo)
            elif None != stOutFrame.pBufAddr and 0 == ret and stOutFrame.stFrameInfo.enPixelType == 17301513:    # BayerRG8 格式
                print("get one frame: Width[%d], Height[%d], nFrameNum[%d]" % (stOutFrame.stFrameInfo.nWidth, stOutFrame.stFrameInfo.nHeight, stOutFrame.stFrameInfo.nFrameNum))
                pData = (c_ubyte * stOutFrame.stFrameInfo.nWidth * stOutFrame.stFrameInfo.nHeight)()
                cdll.msvcrt.memcpy(byref(pData), stOutFrame.pBufAddr,stOutFrame.stFrameInfo.nWidth * stOutFrame.stFrameInfo.nHeight)
                data = np.frombuffer(pData, count=int(stOutFrame.stFrameInfo.nWidth * stOutFrame.stFrameInfo.nHeight),dtype=np.uint8)
    ...
    elif active_way == "getimageforRGB":
        stParam = MVCC_INTVALUE_EX()
        memset(byref(stParam), 0, sizeof(MVCC_INTVALUE_EX))
        ret = cam.MV_CC_GetIntValueEx("PayloadSize", stParam)
        if ret != 0:
        ...
        memset(byref(stFrameInfo), 0, sizeof(stFrameInfo))
        while True:
            ret = cam.MV_CC_GetImageForRGB(pData, nDataSize, stFrameInfo, 1000)
            if ret == 0:
                print("get one frame: Width[%d], Height[%d], nFrameNum[%d] " % (stFrameInfo.nWidth, stFrameInfo.nHeight, stFrameInfo.nFrameNum))
                image = np.asarray(pData)
                image_control(data=image, stFrameInfo=stFrameInfo)
    ...
    elif active_way == "getimageforBGR":
        stParam = MVCC_INTVALUE_EX()
        memset(byref(stParam), 0, sizeof(MVCC_INTVALUE_EX))
        ret = cam.MV_CC_GetIntValueEx("PayloadSize", stParam)
        if ret != 0:
        ...
        memset(byref(stFrameInfo), 0, sizeof(stFrameInfo))
        while True:
            ret = cam.MV_CC_GetImageForBGR(pData, nDataSize, stFrameInfo, 1000)
            if ret == 0:
                print("get one frame: Width[%d], Height[%d], nFrameNum[%d] " % (stFrameInfo.nWidth, stFrameInfo.nHeight, stFrameInfo.nFrameNum))
                image = np.asarray(pData)
                image_control(data=image, stFrameInfo=stFrameInfo)
...
def image_callback(pData, pFrameInfo, pUser):
    ...
    if stFrameInfo:
        print ("get one frame: Width[%d], Height[%d], nFrameNum[%d]" % (stFrameInfo.nWidth, stFrameInfo.nHeight, stFrameInfo.nFrameNum))
    if img_buff is None and stFrameInfo.enPixelType == 17301505:       # Mono8 格式
        ...
        del img_buff
    elif img_buff is None and stFrameInfo.enPixelType == 17301514:     # BayerGB8 格式
        img_buff = (c_ubyte * stFrameInfo.nWidth*stFrameInfo.nHeight)()
        cdll.msvcrt.memcpy(byref(img_buff) , pData , stFrameInfo.nWidth*stFrameInfo.nHeight)
        data = np.frombuffer(img_buff , count = int(stFrameInfo.nWidth*stFrameInfo.nHeight) , dtype = np.uint8)
        image_control(data=data, stFrameInfo=stFrameInfo)
        del img_buff
    elif img_buff is None and stFrameInfo.enPixelType == 35127316:    # RGB8_Packed 格式
        ...
        image_control(data=data, stFrameInfo=stFrameInfo)
        del img_buff
    elif img_buff is None and stFrameInfo.enPixelType == 34603039:    # YUV422_Packed 格式
        ...
        image_control(data=data, stFrameInfo=stFrameInfo)
        del img_buff
    elif img_buff is None and stFrameInfo.enPixelType == 35127317:     # BGR8_Packed 格式
        img_buff = (c_ubyte * stFrameInfo.nWidth * stFrameInfo.nHeight * 3)()
        cdll.msvcrt.memcpy(byref(img_buff), pData, stFrameInfo.nWidth * stFrameInfo.nHeight * 3)
        data = np.frombuffer(img_buff, count=int(stFrameInfo.nWidth * stFrameInfo.nHeight * 3), dtype=np.uint8)
        image_control(data=data, stFrameInfo=stFrameInfo)
        del img_buff
    elif img_buff is None and stFrameInfo.enPixelType == 17301513:     # BayerRG8 格式
        img_buff = (c_ubyte * stFrameInfo.nWidth*stFrameInfo.nHeight)()
        cdll.msvcrt.memcpy(byref(img_buff) , pData , stFrameInfo.nWidth*stFrameInfo.nHeight)
        data = np.frombuffer(img_buff , count = int(stFrameInfo.nWidth*stFrameInfo.nHeight) , dtype = np.uint8)
        image_control(data=data, stFrameInfo=stFrameInfo)
        del img_buff
    elif img_buff is None and stFrameInfo.enPixelType == 34603058:    # YUV_422_YUYV 格式
        img_buff = (c_ubyte * stFrameInfo.nWidth * stFrameInfo.nHeight * 2)()
        cdll.msvcrt.memcpy(byref(img_buff), pData, stFrameInfo.nWidth * stFrameInfo.nHeight * 2)
        data = np.frombuffer(img_buff, count=int(stFrameInfo.nWidth * stFrameInfo.nHeight * 2), dtype=np.uint8)
        image_control(data=data, stFrameInfo=stFrameInfo)
...
# 注册回调取图
def call_back_get_image(cam , active_way = "callbackex"):
    if active_way == "callbackex":
    ...
    elif active_way == "callbackforRGB":
        # ch:注册抓图回调 | en:Register image callback
        ret = cam.MV_CC_RegisterImageCallBackForRGB(CALL_BACK_FUN, None)
        if ret != 0:
            print("register image callback fail! ret[0x%x]" % ret)
            sys.exit()

    elif active_way == "callbackforBGR":
        # ch:注册抓图回调 | en:Register image callback
...
def main():
    # # 需要使用哪种方式连接相机
    # info = input("先枚举所有相机，再连接相机请输入 0    指定 IP 方式连接相机请输入 1:")
    # if int(info) == 0:
    #     # 枚举设备
    ...
    # if int(stdcall) == 0:
    #     # 回调方式抓取图像
    #     call_back_get_image(cam , active_way="callbackforBGR")
    #     # 开启设备取流
    ...
    for i in range(10):
        ...
        print("11111111")
        if ret != 0:
            print("set Command Value! ret[0x%x]" % ret)
            sys.exit()
